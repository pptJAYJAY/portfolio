Hi, I'm Ploypapat 👋

"As the digital world becomes automated and intelligent, I mainly focus on building impactful projects for humans."*

🎓 Computer Engineering · Kasetsart University (1st Year)
🤖 Super AI Engineering Season 6
🔬 Research: Deep Learning · Quantum Computing · Embedded AI

Featured Projects
- 🧬 AI-Assisted Histology** - VGG16 CNN, 100% accuracy · Presented @ ISTEM-ED 2025 
- 📦 AI Delivery Protector** - HuskyLens + micro:bit · 🥉 WRG Thailand 2024
- ⚛️ Quantum Binary Arithmetic - Research @ 2B-KMUTT Future Leader Camp

Skills
`Python` `Deep Learning` `CNN` `Computer Vision` `Quantum Computing`
`Embedded Systems` `SolidWorks` `Research & Academic Writing`

Connect
📧 ploypapatnimitbunanan@gmail.com · 🌐 [Portfolio](https://pptJAYJAYt.github.io)
